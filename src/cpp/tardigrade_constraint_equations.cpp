...
namespace tardigradeBalanceEquations{
    ...
    namespace constraintEquations{
        ...
        void computeInternalEnergyConstraint(
            ...
            result = ( density * ( *( material_response_begin + predicted_internal_energy_index ) ) - internal_energy ) * test_function;
        ...
        void computeInternalEnergyConstraint(
             ...
             * \param &dRdVF_begin: The starting iterator of the derivative of the residual w.r.t. the phase volume fraction
             * \param &dRdVF_end: The stopping iterator of the derivative of the residual w.r.t. the phase volume fraction
            ...
            *( dRdE_begin + phase ) -= test_function * interpolation_function;
            ...
            for ( auto p = std::pair< unsigned int, dRdVF_iter >( 0, dRdVF_begin ); p.second != dRdVF_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * volume_fraction_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * volume_fraction_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
        ...
        void computeInternalEnergyConstraint(
            ...
            *( dRdRho_begin + phase ) += test_function * ( *( material_response_begin + predicted_internal_energy_index ) ) * interpolation_function;
            *( dRdE_begin + phase ) -= test_function * interpolation_function;
            ...
            for ( auto p = std::pair< unsigned int, dRdRho_iter >( 0, dRdRho_begin ); p.second != dRdRho_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * density_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * density_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
            ...
            for ( auto p = std::pair< unsigned int, dRdU_iter >( 0, dRdU_begin ); p.second != dRdU_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * velocity_index + p.first ) ) * interpolation_function * dUDotdU;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * velocity_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) ) * dUDotdU;
            ...
            for ( auto p = std::pair< unsigned int, dRdW_iter >( 0, dRdW_begin ); p.second != dRdW_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * displacement_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * displacement_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
            ...
            for ( auto p = std::pair< unsigned int, dRdTheta_iter >( 0, dRdTheta_begin ); p.second != dRdTheta_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * temperature_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * temperature_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
            ...
            for ( auto p = std::pair< unsigned int, dRdE_iter >( 0, dRdE_begin ); p.second != dRdE_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * internal_energy_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * internal_energy_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
            ...
            for ( auto p = std::pair< unsigned int, dRdVF_iter >( 0, dRdVF_begin ); p.second != dRdVF_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * volume_fraction_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * volume_fraction_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
            ...
            for ( auto p = std::pair< unsigned int, dRdZ_iter >( 0, dRdZ_begin ); p.second != dRdZ_end; ++p.first, ++p.second ){
                ...
                *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + nphases * additional_dof_index + p.first ) ) * interpolation_function;

                for ( unsigned int a = 0; a < material_response_dim; ++a ){

                    *p.second += test_function * density * ( *( material_response_jacobian_begin + ( nphases * num_phase_dof + num_additional_dof ) * ( 1 + material_response_dim ) * ( predicted_internal_energy_index ) + ( nphases * num_phase_dof + num_additional_dof ) + material_response_dim * ( nphases * additional_dof_index + p.first ) + a ) ) * ( *( interpolation_function_gradient_begin + a ) );
        ...
        void computeInternalEnergyConstraint(
            ...
            for ( auto v = std::pair< unsigned int, result_iter >( 0, result_begin ); v.second != result_end; ++v.first, ++v.second ){
                ...
                computeInternalEnergyConstraint<
                    ...
                    dRdTheta_iter, dRdE_iter, dRdVF_iter,
                    dRdZ_iter, dRdUMesh_iter,
                    density_index        ,
                    displacement_index   ,
                    velocity_index       ,
                    temperature_index    ,
