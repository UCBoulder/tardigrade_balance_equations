...
template<
    int dim, int num_nodes,
    class xi_in,
    class v_tp1_in,   class l_tp1_in,
    class umesh_tp1_in,
    class X_in,
    class value_out
>
void evaluate_at_nodes(
    const xi_in &xi_begin, const xi_in &xi_end,
    const v_tp1_in &v_tp1_begin, const v_tp1_in &v_tp1_end,
    const l_tp1_in &l_tp1_begin, const l_tp1_in &l_tp1_end,
    ...
    std::array<
        typename std::iterator_traits<v_tp1_in>::value_type, dim
    > v_tp1_p;

    std::array<
        typename std::iterator_traits<l_tp1_in>::value_type, 1
    > l_tp1_p;

    std::array<
        typename std::iterator_traits<l_tp1_in>::value_type, 1 * dim
    > grad_l_tp1_p;
    ...
    e.InterpolateQuantity(
        xi_begin, xi_end, l_tp1_begin, l_tp1_end,
        std::begin( l_tp1_p ), std::end( l_tp1_p )
    );

    e.GetGlobalQuantityGradient(
        xi_begin, xi_end, l_tp1_begin, l_tp1_end,
        std::begin( grad_l_tp1_p ), std::end( grad_l_tp1_p )
    );
    ...
    for ( unsigned int i = 0; i < num_nodes; ++i ){
        ...
        tardigradeBalanceEquations::surfaceGrowth::computeSurfaceGrowthBalance(
            ...
            value_begin + dim * i, value_begin + dim * ( i + 1 ) 
        );
    ...
    for ( unsigned int i = 0; i < num_nodes; ++i ){
        ...
        for ( unsigned int j = 0; j < num_nodes; ++j ){
            ...
            tardigradeBalanceEquations::surfaceGrowth::computeSurfaceGrowthBalance(
                ...
                l_tp1_p[ 0 ], std::cbegin( grad_l_tp1_p ), std::cend( grad_l_tp1_p ),
                Ns[ i ], std::cbegin( dNdx ) + dim * i, std::cbegin( dNdx ) + dim * ( i + 1 ),
                Ns[ j ], std::cbegin( dNdx ) + dim * j, std::cbegin( dNdx ) + dim * ( j + 1 ),
                std::begin( value_n ),    std::end( value_n ),
                std::begin( dRdV_n ),     std::end( dRdV_n ),
                std::begin( dRdL_n ),     std::end( dRdL_n ),
            ...
            for( unsigned int k = 0; k < dim; ++k ){
                ...
                *( dRdL_begin + dim * num_nodes * 1 * i + num_nodes * 1 * k + 1 * j + 0 )
                    += dRdL_n[ k ] * J;
...
void evaluate_at_nodes(
    ...
    std::array<
        typename std::iterator_traits<v_tp1_in>::value_type, dim * dim
    > grad_v_tp1_p;
    ...
    e.GetGlobalQuantityGradient(
        xi_begin, xi_end, v_tp1_begin, v_tp1_end,
        std::begin( grad_v_tp1_p ), std::end( grad_v_tp1_p )
    );
...
BOOST_AUTO_TEST_CASE( test_computeSurfaceGrowthBalance2, * boost::unit_test::tolerance( DEFAULT_TEST_TOLERANCE ) ){
    ...
    std::array< floatType, dim * num_nodes > v = {
        ...
        -0.39175842, -0.16595558,  0.36260153,  0.75091368,  0.02084467,
         0.33862757,  0.17187311,  0.249807  ,  0.3493781 
    };

    std::array< floatType, 1 * num_nodes > l = {
         0.68468488, -0.83361002,  0.52736568, -0.51266725, -0.61155408,
         0.14491391, -0.80857497,  0.77065365
    };

    std::array< floatType, dim * num_nodes > u_mesh = {
        0.34317802, 0.72904971, 0.43857224, 0.32179195, 0.7509966 ,
        0.45705522, 0.2944771 , 0.74330725, 0.45514841, 0.31586316,
        0.72136035, 0.43666543, 0.34830949, 0.77712613, 0.427784  ,
        0.32692343, 0.79907302, 0.44626697, 0.29960857, 0.79138367,
        0.44436016, 0.32099464, 0.76943677, 0.42587719
    };

    std::array< floatType, dim * num_nodes > X = {
...
BOOST_AUTO_TEST_CASE( test_computeLagrangeMultiplierBalance2, * boost::unit_test::tolerance( DEFAULT_TEST_TOLERANCE ) ){
    ...
    std::array< floatType, 1 * num_nodes > answer = {
        -3.519554977e-03, -1.922568427e-02, +4.802186299e-02, -1.498709318e-02, -3.256041881e-03, -1.694175527e-02,
