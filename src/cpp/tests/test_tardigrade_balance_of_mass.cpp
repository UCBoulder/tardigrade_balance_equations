    ...
    for ( unsigned int i = 0; i < node_count; ++i ){
        ...
        if ( active_phase >= 0 ){
            ...
            tardigradeBalanceEquations::balanceOfMass::computeDiffusionTerm<10>(
                std::cbegin( material_response ) + material_response_size * j,
                std::cend( material_response ) + material_response_size * ( j + 1 ),
                std::begin( dNdxs ) + dim * i, std::end( dNdxs ) + dim * ( i + 1 ),
...
//
//            // Multiphase evaluation
//            tardigradeBalanceEquations::balanceOfMass::computeDiffusionTerm<dim,10>(
//                std::cbegin( density_tp1_p ),                        std::cend( density_tp1_p ),
//                std::cbegin( density_dot_tp1_p ),                    std::cend( density_dot_tp1_p ),
//                std::cbegin( grad_density_tp1 ),                     std::cend( grad_density_tp1 ),
//                std::cbegin( v_tp1_p ),                              std::cend( v_tp1_p ),
...
BOOST_AUTO_TEST_CASE( test_computeBalanceOfMass_hydra_diffusionTerm_fea, * boost::unit_test::tolerance( 1e-5 ) ){
    ...
    std::array< floatType, 8 * nphases > answer = {
        2.0225999 ,  2.13312325,  1.96621389,  2.16975466,  1.27562866,
        1.34963342,  1.24416065,  1.37628207,  0.84654155,  0.88690599,
        0.83301   ,  0.90679906,  1.38363167,  1.44610739,  1.35433113,
        1.47342163,  1.89159261,  2.06417481,  1.72871356,  2.04973007,
       -1.1294061 , -1.08203022, -1.12080895, -1.08868783, -3.83340187,
       -4.08281776, -3.65347751, -4.11843369, -2.45718644, -2.71509687,
       -2.35214277, -2.76886596
    };
