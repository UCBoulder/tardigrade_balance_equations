/**
 ******************************************************************************
 * \file tardigrade_balance_of_volume_fraction.tpp
 ******************************************************************************
 * The template file for the equations associated with the balance of the
 * volume fraction
 ******************************************************************************
 */
...
namespace tardigradeBalanceEquations {
    ...
    namespace balanceOfVolumeFraction {
        ...
        void computeBalanceOfVolumeFraction(
            ...
            const volume_fraction_gradient_iter &volume_fraction_gradient_end,
            const mass_change_rate_type &mass_change_rate, const rest_density_type &rest_density,
            const trace_mass_change_velocity_gradient_type &trace_mass_change_velocity_gradient,
            const test_function_type &test_function, result_type &result, const double volume_fraction_tolerance) {
            /*!
             ...
             * \param &volume_fraction_dot: The partial time derivative of the current volume fraction \f$ \left(
             * \frac{\partial \phi^{\alpha}}{\partial t} \right) \f$
             * \param &volume_fraction_gradient_begin: The starting iterator of the spatial gradient of the volume
             * fraction \f$ \left( \phi_{,i}^{\alpha} \right) \f$
             * \param &volume_fraction_gradient_end: The stopping iterator of the spatial gradient of the volume
             * fraction \f$ \left( \phi_{,i}^{\alpha} \right) \f$
             * \param &mass_change_rate: The current mass change rate per current volume \f$ \left( c^{\alpha} \right)
             * \f$
             * \param &rest_density: The rest density of the material \f$ \left( \bar{\gamma}^{\alpha} \right) \f$
             * \param &trace_mass_change_velocity_gradient: The current mass change rate per current volume \f$ \left(
             * c^{\alpha} \right) \f$
             * \param &test_function: The current value of the test function
             * \param &result: The value of the balance of volume fraction
             * \param volume_fraction_tolerance: The tolerance of the volume fraction where if it is less than the
        ...
        void computeBalanceOfVolumeFractionFullMaterialResponse(
            const density_type &density, const velocity_iter &velocity_begin, const velocity_iter &velocity_end,
            const volume_fraction_type &volume_fraction, const volume_fraction_dot_type &volume_fraction_dot,
            const volume_fraction_gradient_iter &volume_fraction_gradient_begin,
            const volume_fraction_gradient_iter &volume_fraction_gradient_end,
             ...
             * \param &volume_fraction_gradient_end: The stopping iterator of the spatial gradient of the volume
             * fraction \f$ \left( \phi_{,i}^{\alpha} \right) \f$
             * \param &material_response_begin: The starting iterator for the material response vector. This vector
             * defines the mass change rate and the trace of the mass-change velocity gradient which are located at
             * their respective indices (see template parameters)
             * \param &material_response_end: The stopping iterator for the material response vector. This vector
             * defines the mass change rate and the trace of the mass-change velocity gradient which are located at
             * their respective indices (see template parameters)
             * \param &rest_density: The rest density of the material \f$ \left( \bar{\gamma}^{\alpha} \right) \f$
             * \param &test_function: The current value of the test function
            ...
            computeBalanceOfVolumeFraction<configuration>(
                ...
                *(material_response_begin + configuration::material::output::mass_change_rate_index), rest_density,
                *(material_response_begin + configuration::material::output::trace_mass_change_velocity_gradient_index),
        ...
        template <class configuration, typename density_type, class velocity_iter, typename volume_fraction_type,
                  ...
                  typename dUDotdU_type, typename dVolumeFractionDotdVolumeFraction_type, typename result_type,
                  typename dRdRho_type, class dRdU_iter, typename dRdVolumeFraction_type, typename dRdC_type,
                  typename dRdTraceVA_type, class dRdUMesh_iter>
        void computeBalanceOfVolumeFraction(
            const density_type &density, const velocity_iter &velocity_begin, const velocity_iter &velocity_end,
            const volume_fraction_type &volume_fraction, const volume_fraction_dot_type &volume_fraction_dot,
            const volume_fraction_gradient_iter &volume_fraction_gradient_begin,
            ...
            const interpolation_function_gradient_iter &interpolation_function_gradient_end, const dUDotdU_type dUDotdU,
            const dVolumeFractionDotdVolumeFraction_type dVolumeFractionDotdVolumeFraction, result_type &result,
            dRdRho_type &dRdRho, dRdU_iter dRdU_begin, dRdU_iter dRdU_end, dRdVolumeFraction_type &dRdVolumeFraction,
             ...
             * \param &dVolumeFractionDotdVolumeFraction: The derivative of the partial time derivative of the volume
             * fraction phase w.r.t. the volume fraction
             * \param &result: The value of the balance of volume fraction
             * \param &dRdRho: The derivative of the residual w.r.t. the apparent density
             * \param &dRdU_begin: The starting iterator of the derivative of the residual w.r.t. the phase displacement
             * dof
             * \param &dRdU_end: The stopping iterator of the derivative of the residual w.r.t. the phase displacement
             * dof
             ...
             * \param &dRdTraceVA: The derivative of the residual w.r.t. the trace of the velocity gradient associated
            ...
            dRdVolumeFraction =
                interpolation_function * (dVolumeFractionDotdVolumeFraction + trace_mass_change_velocity_gradient);
            ...
            for (unsigned int i = 0; i < configuration::dimension; ++i) {
                ...
                *(dRdU_begin + i) +=
                    test_function * (*(volume_fraction_gradient_begin + i)) * interpolation_function * dUDotdU;
        ...
        void computeBalanceOfVolumeFraction(
            ...
            const rest_density_type &rest_density, const test_function_type &test_function,
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(dRdTheta_end - dRdTheta_begin),
                                         "The dRdTheta must be the same size as the number of phases")

            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(dRdE_end - dRdE_begin),
                                         "The dRdE must be the same size as the number of phases")
            ...
            computeBalanceOfVolumeFraction<configuration>(
                ...
                *(material_response_begin + configuration::material::output::trace_mass_change_velocity_gradient_index),
                ...
                interpolation_function_gradient_end, dUDotdU, dVolumeFractionDotdVolumeFraction, result,
                *(dRdRho_begin + phase), dRdU_begin + configuration::dimension * phase,
                dRdU_begin + configuration::dimension * (phase + 1), *(dRdVolumeFraction_begin + phase), dRdC_phase,
                dRdTraceVA_phase, dRdUMesh_begin, dRdUMesh_end, volume_fraction_tolerance);
            ...
            for (auto p = std::pair<unsigned int, dRdRho_iter>(0, dRdRho_begin); p.second != dRdRho_end;
                 ++p.first, ++p.second) {
                *p.second += dRdTraceVA_phase *
                             (*(material_response_jacobian_begin +
                                (nphases * configuration::material::dof::num_phase_dof +
                                 configuration::material::dof::num_additional_dof) *
                                    (1 + configuration::material::dimension) *
                                    (configuration::material::output::trace_mass_change_velocity_gradient_index) +
                                nphases * configuration::material::dof::density_index + p.first)) *
                             interpolation_function;
        ...
        void computeBalanceOfVolumeFractionMultiphase(
            ...
            const volume_fraction_dot_iter      &volume_fraction_dot_end,
            const volume_fraction_gradient_iter &volume_fraction_gradient_begin,
            const volume_fraction_gradient_iter &volume_fraction_gradient_end,
             ...
             *
             * \param &density_begin: The starting iterator of the current apparent density \f$ \left( \rho^{\alpha}
             * \right) \f$
             * \param &density_end: The stopping iterator of the current apparent density \f$ \left( \rho^{\alpha}
             * \right) \f$
             * \param &velocity_begin: The starting iterator of the phase velocity \f$ \left( v_i^{\alpha} \right) \f$
             * \param &velocity_end: The stopping iterator of the phase velocity \f$ \left( v_i^{\alpha} \right) \f$
             * \param &volume_fraction_begin: The starting iterator of the current volume fraction \f$ \left(
             * \phi^{\alpha} \right) \f$
             * \param &volume_fraction_end: The stopping iterator of the current volume fraction \f$ \left(
             * \phi^{\alpha} \right) \f$
             * \param &volume_fraction_dot_begin: The starting iterator of the partial time derivative of the current
             * volume fraction \f$ \left( \frac{\partial \phi^{\alpha}}{\partial t} \right) \f$
             * \param &volume_fraction_dot_end: The stopping iterator of the partial time derivative of the current
             * volume fraction \f$ \left( \frac{\partial \phi^{\alpha}}{\partial t} \right) \f$
             * \param &volume_fraction_gradient_begin: The starting iterator of the spatial gradient of the volume
             * fraction \f$ \left( \phi_{,i}^{\alpha} \right) \f$
             * \param &volume_fraction_gradient_end: The stopping iterator of the spatial gradient of the volume
             * fraction \f$ \left( \phi_{,i}^{\alpha} \right) \f$
             * \param &mass_change_rate_begin: The starting iterator of the current mass change rate per current volume
             * \f$ \left( c^{\alpha} \right) \f$
             * \param &mass_change_rate_end: The stopping iterator of the current mass change rate per current volume
             * \f$ \left( c^{\alpha} \right) \f$
             * \param &rest_density_begin: The starting iterator of the rest density of the material \f$ \left(
             * \bar{\gamma}^{\alpha} \right) \f$
             * \param &rest_density_end: The stopping iterator of the rest density of the material \f$ \left(
             * \bar{\gamma}^{\alpha} \right) \f$
             * \param &trace_mass_change_velocity_gradient_begin: The starting iterator of the current mass change rate
             * per current volume \f$ \left( c^{\alpha} \right) \f$
             * \param &trace_mass_change_velocity_gradient_end: The stopping iterator of the current mass change rate
             * per current volume \f$ \left( c^{\alpha} \right) \f$
             * \param &test_function: The current value of the test function
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(configuration::dimension * nphases ==
                                             (unsigned int)(volume_fraction_gradient_end -
                                                            volume_fraction_gradient_begin),
                                         "The volume fraction gradient size is inconsistent with the number of phases");

            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(mass_change_rate_end - mass_change_rate_begin),
                                         "The mass change rate size is not equal to the number of phases");

            TARDIGRADE_ERROR_TOOLS_CHECK(
                nphases ==
                    (unsigned int)(trace_mass_change_velocity_gradient_end - trace_mass_change_velocity_gradient_begin),
            ...
            for (auto rho = std::pair<unsigned int, density_iter>(0, density_begin); rho.second != density_end;
                ...
                computeBalanceOfVolumeFraction<configuration>(
                    *rho.second, velocity_begin + configuration::dimension * rho.first,
                    velocity_begin + configuration::dimension * (rho.first + 1), *(volume_fraction_begin + rho.first),
                    *(volume_fraction_dot_begin + rho.first),
                    ...
                    *(mass_change_rate_begin + rho.first), *(rest_density_begin + rho.first),
                    *(trace_mass_change_velocity_gradient_begin + rho.first), test_function,
                    *(result_begin + rho.first), volume_fraction_tolerance);
            }
        }
        ...
        void computeBalanceOfVolumeFractionMultiphase(
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(
                (material_response_size > configuration::material::output::mass_change_rate_index),
                "The material response vector size must larger than the index for the mass change rate")

            TARDIGRADE_ERROR_TOOLS_CHECK((material_response_size >
                                          configuration::material::output::trace_mass_change_velocity_gradient_index),
                                         "The material response vector size must larger than the index for the trace "
                                         "of the mass change rate velocity gradient")

            for (auto rho = std::pair<unsigned int, density_iter>(0, density_begin); rho.second != density_end;
                 ++rho.first, ++rho.second) {
                computeBalanceOfVolumeFractionFullMaterialResponse<configuration>(
                    *rho.second, velocity_begin + configuration::dimension * rho.first,
                    velocity_begin + configuration::dimension * (rho.first + 1), *(volume_fraction_begin + rho.first),
                    *(volume_fraction_dot_begin + rho.first),
                    volume_fraction_gradient_begin + configuration::dimension * rho.first,
        ...
        void computeBalanceOfVolumeFractionMultiphase(
            ...
            const interpolation_function_gradient_iter &interpolation_function_gradient_begin,
            const interpolation_function_gradient_iter &interpolation_function_gradient_end, const dUDotdU_type dUDotdU,
            const dVolumeFractionDotdVolumeFraction_type dVolumeFractionDotdVolumeFraction, result_iter result_begin,
            result_iter result_end, dRdRho_iter dRdRho_begin, dRdRho_iter dRdRho_end, dRdU_iter dRdU_begin,
            dRdU_iter dRdU_end, dRdVolumeFraction_iter dRdVolumeFraction_begin,
            dRdVolumeFraction_iter dRdVolumeFraction_end, dRdC_iter dRdC_begin, dRdC_iter dRdC_end,
            dRdTraceVA_iter dRdTraceVA_begin, dRdTraceVA_iter dRdTraceVA_end, dRdUMesh_iter dRdUMesh_begin,
            dRdUMesh_iter dRdUMesh_end, const double volume_fraction_tolerance) {
            /*!
             * Compute the balance of the volume fraction for a multiphase reacting continuum
             *
             * \param &density_begin: The starting iterator of the current apparent density \f$ \left( \rho^{\alpha}
             ...
             * \param &test_function: The current value of the test function
             * \param &interpolation_function: The current value of the interpolation function
             * \param &interpolation_function_gradient_begin: The starting iterator of the current value of the
             * interpolation function
             * \param &interpolation_function_gradient_end: The stopping iterator of the current value of the
             * interpolation function
             * \param &dUDotdU: The derivative of the phase velocity w.r.t. the phase displacement dof
             * \param &dVolumeFractionDotdVolumeFraction: The derivative of the partial time derivative of the volume
             * fraction phase w.r.t. the volume fraction
             ...
             * \param &dRdVolumeFraction_end: The stopping iterator of the derivative of the residual w.r.t. the volume
             * fraction
             * \param &dRdC_begin: The starting iterator of the derivative of the residual w.r.t. the mass change rate
             * per unit volume
             * \param &dRdC_end: The stopping iterator of the derivative of the residual w.r.t. the mass change rate per
             * unit volume
             * \param &dRdTraceVA_begin: The starting iterator of the derivative of the residual w.r.t. the trace of the
             * velocity gradient associated with the change in mass
             * \param &dRdTraceVA_end: The stopping iterator of the derivative of the residual w.r.t. the trace of the
             * velocity gradient associated with the change in mass
             * \param &dRdUMesh_begin: The starting iterator of the derivative of the residual w.r.t. the mesh
             * displacement
             * \param &dRdUMesh_end: The stopping iterator of the derivative of the residual w.r.t. the mesh
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(dRdC_end - dRdC_begin),
            ...
            for (auto rho = std::pair<unsigned int, density_iter>(0, density_begin); rho.second != density_end;
                ...
                computeBalanceOfVolumeFraction<configuration>(
                    ...
                    *(volume_fraction_dot_begin + rho.first),
                    ...
                    volume_fraction_gradient_begin + configuration::dimension * (rho.first + 1),
                    *(mass_change_rate_begin + rho.first), *(rest_density_begin + rho.first),
                    *(trace_mass_change_velocity_gradient_begin + rho.first), test_function, interpolation_function,
                    interpolation_function_gradient_begin, interpolation_function_gradient_end, dUDotdU,
                    dVolumeFractionDotdVolumeFraction, *(result_begin + rho.first), *(dRdRho_begin + rho.first),
                    ...
                    *(dRdC_begin + rho.first), *(dRdTraceVA_begin + rho.first),
        ...
        void computeBalanceOfVolumeFractionMultiphase(
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(volume_fraction_end - volume_fraction_begin),
                                         "The volume fraction must have the same size as the result vector")

            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(volume_fraction_dot_end - volume_fraction_dot_begin),
                                         "The volume fraction dot must have the same size as the result vector")

            TARDIGRADE_ERROR_TOOLS_CHECK(
                configuration::dimension * nphases ==
                    (unsigned int)(volume_fraction_gradient_end - volume_fraction_gradient_begin),
                "The patial gradient of the volume fraction must be consistent with the size of the result vector")

            TARDIGRADE_ERROR_TOOLS_CHECK(
                nphases * material_response_size == (unsigned int)(material_response_end - material_response_begin),
                "The material response vector size must be an integer multiple of the number of phases")
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(nphases * material_response_size *
                                             ...
                                             (unsigned int)(material_response_jacobian_end -
                                                            material_response_jacobian_begin),
                                         "The material response Jacobian vector size must be consistent with the "
                                         "number of phases and the number of DOF in the material repsonse")

            TARDIGRADE_ERROR_TOOLS_CHECK(nphases == (unsigned int)(rest_density_end - rest_density_begin),
                                         "The rest density must have the same size as the result vector")

            TARDIGRADE_ERROR_TOOLS_CHECK((nphases * configuration::material::dof::num_phase_dof +
                                          configuration::material::dof::num_additional_dof) *
                                                 configuration::material::dimension ==
            ...
            TARDIGRADE_ERROR_TOOLS_CHECK(nphases * nphases == (dRdE_end - dRdE_begin),
                                         "The dRdE must be the same size as the number of phases")
            ...
            for (auto v = std::pair<unsigned int, result_iter>(0, result_begin); v.second != result_end;
                 ++v.first, ++v.second) {
                computeBalanceOfVolumeFraction<configuration>(
                    *(density_begin + v.first), velocity_begin + configuration::dimension * v.first,
                    velocity_begin + configuration::dimension * (v.first + 1), *(volume_fraction_begin + v.first),
                    ...
                    *(rest_density_begin + v.first), test_function, interpolation_function,
